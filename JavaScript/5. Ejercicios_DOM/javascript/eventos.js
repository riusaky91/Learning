  ...
  export default function eventos($esfera, $escenario){
    ...
    let $limiteEsfera = $esfera.getBoundingClientRect();//obteniendo los limites del elemento esfera
    let $limiteEscenario = $escenario.getBoundingClientRect();//obteniendo los limites del elemento escenario
    let y = 0, x = 0;
  ...
  d.addEventListener("keydown", (e)=>{//evento que se ejecuta cuando presiono una tecla en el teclado
    ...
    if(e.key === "ArrowLeft"){//si presiono la flecha izquierda
      ...
      e.preventDefault()//Metodo que elimina todos los eventos por defecto del navegador
      if($limiteEsfera>$limiteEscenario) console.log("adentro");
